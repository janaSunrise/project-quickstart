...
def get_project_path() -> str:
    path = inquirer.text(
        message=f"{Style.BRIGHT}{Fore.YELLOW}Enter the path to your project destination"
    )
...
def create_readme(project_name: str, lang: str, project_path: str) -> None:
    ...
    with console.status(
        f"{Style.BRIGHT}{Fore.YELLOW}Initializing readme...", spinner="dots"
    ):
        time.sleep(1)
...
def create_source(lang: str, lang_extension: str, project_path: str) -> None:
    with console.status(
        f"{Style.BRIGHT}{Fore.YELLOW}Initializing main{lang_extension}...",
        spinner="dots",
    ):
        time.sleep(1)
...
def create_gitignore(lang: str, project_path: str) -> bool:
    with console.status(
        f"{Style.BRIGHT}{Fore.YELLOW}Creating .gitignore ...", spinner="dots"
    ):
        time.sleep(1)
        ...
        if req.status_code != 200:
            print(
                f"{Style.BRIGHT}{Fore.RED}Couldn't initialize gitignore due to Network Error!"
            )
            return False
...
def create_license(name: str, project_path: str) -> bool:
    ...
    with console.status(
        f"{Style.BRIGHT}{Fore.YELLOW}Fetching license...", spinner="dots"
    ):
        time.sleep(1)
        ...
        with console.status(
            f"{Style.BRIGHT}{Fore.YELLOW}Initializing License[{name}]", spinner="dots"
        ):
            time.sleep(1)
...
def git_init(project_path: str) -> None:
    with console.status(
        f"{Style.BRIGHT}{Fore.GREEN}Finalizing the project...", spinner="dots"
    ):
        try:
